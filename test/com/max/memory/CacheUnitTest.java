...
class CacheUnitTest {
	...
	public static DaoFileImpl<Integer> dao = new DaoFileImpl<>("DataSource.txt", CAPACITY * 2);
	...
	static void setUpWithData() {
		for (int i = 0; i < CAPACITY; i++) {
			...
			assertEquals(dataModels[i].getDataModelId(), dao.find(dataModels[i].getDataModelId()).getDataModelId(),
					"if inserted to the file it should find the current model");
		}
	}
	...
	static void afterAll() {
		for (int i = 0; i < CAPACITY; i++) {
			...
			assertEquals(null, dao.find(dataModels[i].getDataModelId()),
					"after we finished the test, we delete all models , if deleted successfully should return null");
